...
```{r mapWHO, fig.align = 'center', out.width= "80%"}
...
```{r mapWHOepicurve,,fig.align='center', out.width= "90%"}
knitr::include_graphics("../input_files/WHO_Africa_epicurve.png")
```
...
# add column signifying WR related to chosen country and remove countries with incomplete data and only data within last 8 days
 ...
 current_country_WR_deaths <-  WHO_cases_and_deaths_weekly_ratio %>%
  ...
  cases_per_10k_plot <- ggplot() +
    ...
    scale_y_log10(breaks = c(100,10,1,0.1,0.01,0.001,0.0001),labels = c("100","10","1","0.1","0.01","0.001","0.0001")) +
    scale_x_date(date_labels = "%b %d",breaks=c(today-140,today-112,today-84,today-56,today-28,today)) +
  ...
  deaths_per_10k_plot <- ggplot() +
    ...
    geom_line(data=current_country_data %>% filter(current_country,cum_deaths_per_10k>0),
              ...
              colour="black",size = 0.8) +
    scale_y_log10(breaks = c(10,1,0.1,0.01,0.001,0.0001),labels = c("10","1","0.1","0.01","0.001","0.0001")) +
    theme_classic(base_size = 15) +
    theme(panel.grid.major.y = element_line(colour = "lightgrey",size=0.02),
          axis.title.x = element_blank(),
          axis.title.y = element_blank(),
...
Confidence intervals for the ratios (w1/w2) were calculated based on the approximate standard error for their logs,
sqrt{1/w1 + 1/w2}, where w1 = total cases in the past week and w2 = total cases in the week prior.
